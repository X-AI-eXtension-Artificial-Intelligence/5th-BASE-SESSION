...
fr_encoding = detect_encoding("data/training/train.fr")
en_encoding = detect_encoding("data/training/train.en")

print(f"French file encoding: {fr_encoding}")
print(f"English file encoding: {en_encoding}")

# 훈련 데이터 불러오기
# Fr -> En 번역을 위한 데이터셋(Multi-30k) 활용
fr_train = utils.open_text_set("data/training/train.fr", encoding=fr_encoding)
en_train = utils.open_text_set("data/training/train.en", encoding=en_encoding)

# Vocab 만들기 / 관련 함수는 utils.py 참조
try:
    vocab_transform, token_transform = utils.make_vocab(fr_train, en_train)
except Exception as e:
    print(f"Error during vocab creation: {e}")
