...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        # 각 attention 결과 모음
        # Scaled-Dot-Product Attention head 수 만큼 반복
        self_attentions = [attention(query, key, value, mask) for attention in self.attentions]

        weighted_vs = [weighted_v[0] for weighted_v in self_attentions] # 각 self.dropout(weighted_v) 모음 = Scaled-Dot-Product Attention들의 weighted_v 모음
        attentions = [weighted_v[1] for weighted_v in self_attentions] # 각 attention_score 모음 = Scaled-Dot-Product Attention들의 attention_score 모음
...
class SelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # q, k matmul
        self_attention = torch.bmm(q, k.permute(0, 2, 1)) # k.permute(0, 2, 1) = transpose한 것 / torch.bmm = matmul과 유사하나 broadcast 안 됨
        # Scale
        ...
        # 정규화한 attention score(가중치)와 Value matrix 계산 / matmul
