...
if mode == 'train':
    ...
    for epoch in range(st_epoch + 1, num_epoch + 1):
        ...
        for batch, data in enumerate(loader_train, 1):
            ...
            # 타겟 라벨의 크기를 모델의 출력 크기에 맞춰 조정
            label_resized = F.interpolate(label, size=output.shape[2:], mode='bilinear', align_corners=False)

            # 손실 계산
            # 손실 계산 전에 타겟 라벨 크기 조정
            loss = fn_loss(output, label_resized)
            optim.zero_grad()
