...
class InputEmbeddings(nn.Module):
    ...
    def __init__(self,d_model : int, vocab_size : int):
        ...
        #Input Embedding (vocab 사이즈와 차원)
        self.embedding = nn.Embedding(vocab_size, d_model)
        
    def forward(self,x):
        #루트(d차원)을 곱해줌 ("In the embedding layers, we multiply those weights by √dmodel")
...
class PositionalEncoding(nn.Module):
    ...
    #함수 리턴 값의 주석 역할(-> None)
    #해당 함수의 반환 타입의 예상 타입을 나타내기 위해 사용(코드 가독성)
    def __init__(self, d_model : int, seq_len: int, dropout:float) -> None:
        ...
        #빈 텐서 셍성 (seq_len, d_model)
        pe = torch.zeros(seq_len,d_model)

        #row 방향으로 (0~seq_len) 생성 (unsqueeze(dim=1))
        # 단어의 위치를 의미
        position = torch.arange(0,seq_len, dtype = torch.float).unsqueeze(1)

        #col 방향으로 step=2를 활용하여 i의 2배수를 만듦 (0~2i) 
        ...
        #차원 늘리기
        #(tensor의 0번째 공간에 batch 차원을 나타내기 위함임)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    #class 밖에서 선언된 def 함수(정적메서드)
    #굳이 인스턴스를 생성하지 않고도 호출할 수 있음
    ...
    def attention(query, key, value, mask, dropout : nn.Dropout):
        ...
        # 쿼리와 키 간의 내적 값 구하고 --> 스케일링 by sqrt
    ...
    def forward(self, q, k, v, mask):
        ...
        x = x.transpose(1,2).contiguous().view(x.shape[0],-1, self.h*self.d_k) # -1 : 나머지 차원을 자동으로 조정하라는 의미
